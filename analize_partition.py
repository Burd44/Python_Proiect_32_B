...
def plot_chart(extension_size, extension_count):
    ...
    for i in range(1, len(sizes), 25):
        ...
        bars = sp1.bar(keys, values, color='sienna')
        sp1.set_title('File Size and Count Comparison by Extension')
        sp1.set_ylabel('Size (MB)')
        sp1.set_xticks(range(len(keys)))
        sp1.set_xticklabels(keys, rotation=25, ha='right')
        for bar, value in zip(bars, values):
            sp1.text(bar.get_x() + bar.get_width() / 2, bar.get_height(), f'{round(value, 2)}', ha='center', va='bottom')
        chunk = counts[i:i + 25]
        keys, values = zip(*chunk)
        bars = sp2.bar(keys, values, color='royalblue')
        sp2.set_ylabel('Count')
        sp2.set_xticks(range(len(keys)))
        sp2.set_xticklabels(keys, rotation=25, ha='right')
        for bar, value in zip(bars, values):
            sp2.text(bar.get_x() + bar.get_width() / 2, bar.get_height(), f'{(int)(value)}', ha='center', va='bottom')
        plt_button1 = plt.axes([0.89, 0.01, 0.1, 0.03])
        button1 = Button(plt_button1, 'Next')
        button1.on_clicked(next)
        plt_button2 = plt.axes([0.01, 0.01, 0.1, 0.03])
        button2 = Button(plt_button2, 'Quit')
